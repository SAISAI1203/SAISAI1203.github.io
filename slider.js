$(function () {
  ...
  function move() {
    ...
    if (parseInt(p.style.left) > -3000) {
      // 和bannerimage的宽度保持一致即可：700
      p.style.left = parseInt(p.style.left) - 500 + "px";
      p.style.transition = "left 1s";
      tog(-Math.round(parseInt(p.style.left) / 500));
      if (parseInt(p.style.left) <= -3000) {
        setTimeout(function () {
          tog(0);
          p.style.left = "0px";
  ...
  for (var i = 0; i < button.length; i++) {
    ...
    button[i].onclick = function () {
      p.style.left = -500 * this.getAttribute("data-index") + "px";
